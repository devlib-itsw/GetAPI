package com.DevLib.service;

import com.DevLib.domain.SmsAuth;
import com.DevLib.domain.User;
import com.DevLib.repository.SmsAuthRepository;
import com.DevLib.repository.UserStore;
import com.DevLib.util.ImapUtil;
...
public class SmsAuthService {
    ...
    public void saveUser(String name, String phone, String mail, String profileImg) {
        userStore.save(new User(name, phone, mail, profileImg));
    }

    public void saveToken(String token, String userEmail, String userName, String userImg) {
        smsAuthRepository.save(new SmsAuth(token, userEmail, userName, userImg, null));
    }
    ...
    public SmsAuth processSmsAuth(MimeMessage mimeMessage) {
        try {
            ...
            if(domain != "vmms.nate.com" && domain != "mmsmail.uplus.co.kr" && domain != "mmsmail.uplus.co.kr") {
            ...
            // 본문 예: 수정하지말고 그대로 전송해주세요. (diouandnawdniuanwd)
            ...
            if (bodyMatcher.find()) {
                ...
                if (phone.equals(getPhone)) {
                    saveUser(user.getUserName(), getPhone, user.getUserEmail(), user.getUserImg());
